...
function setup() {
  ...
  let canvasHeight = windowHeight * 0.6; // 60% da altura da janela
...
function desenharLinhaPrincipal() {
  let centroX = width / 2;
  let inicioY = height * 0.15;   // ajustado proporcionalmente
  let espacoY = height * 0.12;
  let boxW = width * 0.27;
  ...
  let startY = -boxH * 2.5; // adaptado para não cortar
  ...
  for (let i = 0; i < eventos.length; i++) {
    ...
    stroke(bordaCor);
    strokeWeight(5);
    rect(pos.x, pos.y, boxW, boxH, boxH * 0.33); // raio de canto proporcional
  ...
  stroke(roxo);
  strokeWeight(5);
  line(posUltimo.x, posUltimo.y + boxH / 2, posUltimo.x, height + boxH);
}
